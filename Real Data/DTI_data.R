...
load("DTI.RData")
Data=(DTI$cca)
drop<-unique(which(is.na(Data),arr.ind = T)[,1])
Data<-Data[-drop,] # Missing value
Data<-t(Data)
...
eps_candi <- c(1/16, 1/8, 1/4, 1/2, 1, 2, 4)
...
for(kernel_type in c("M3/2", "M5/2")){
  ...
  MSE_DTI <- matrix(NA, nrow = length(eps_candi), ncol = 4,
  ...
  for(i in 1:length(eps_candi)){
    ...
    MSE_DTI[i,3] <- TMAUTO_res_plug$MSE[3]
    ...
    MSE_DTI[i,4] <- RKHS_res_plug$MSE[3]
    
    
    # MU_tilde[[i]] = cbind(TMIID_res_plug$mu_tilde, bern_res$mu_tilde,
    #                       TMAUTO_res_plug$mu_tilde, RKHS_res_plug$mu_tilde)
  ...
  All_res[[kernel_type]] = list(MSE = MSE_DTI,
                                MU_tilde = MU_tilde)
